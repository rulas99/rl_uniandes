...
case "$RUN_PROFILE" in
  ...
  thesis_lambda_sweep)
    ...
    ;;
  thesis_9x9_full_256)
    # Definitive 5-method comparison on (256,256): ddqn_vanilla, oracle_reset,
    # oracle_segmented, oracle_segmented_distill_l001 (λ=0.001, Option B fix:
    # TD on all samples + distill anchor on archive), and der_plus_plus.
    # This profile carries the final thesis hyperparameters only.
    # The canonical 45-seed split lives in run_full_256_shard{1,2,3}.sh.
    MODE="continual"
