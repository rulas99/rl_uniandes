...
class GridWorldEnv(gym.Env):
    ...
    @staticmethod
    def transform_obs(obs: Dict[str, np.ndarray]) -> np.ndarray:
        # Regresar solo la posicion del agente normalizada [0,1] y en float32
        return obs["agent"].astype(np.float32) / (obs["agent"].shape[0] - 1)
